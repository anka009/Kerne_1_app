...
if uploaded_file:
    ...
    # 🔄 PIL → Base64-URL konvertieren (Canvas benötigt das!)
    buf = io.BytesIO()
    pil_img.save(buf, format="PNG")
    data = base64.b64encode(buf.getvalue()).decode("utf-8")
    img_url = f"data:image/png;base64,{data}"

    # 🖌️ Zeichenmodus wählen
    ...
    # 🧠 Analyse der ZOI (Zone of Interest)
    if canvas_result.json_data and canvas_result.json_data["objects"]:
        ...
        cv2.drawContours(output_roi, contours_red, -1, (255, 0, 0), 2)       # Rot
        cv2.drawContours(output_roi, contours_blue, -1, (173, 216, 230), 2)  # Hellblau

        st.image(output_roi, caption="📍 Hervorgehobene Flecken in ZOI")
    else:
        st.warning("🖼️ Bitte zeichne eine Zone of Interest (ZOI) ins Bild.")
else:
    st.info("🔼 Lade ein Bild hoch, um loszulegen.")
